...
static float consumo_lavatrice = 1.3;
...
PROCESS_THREAD(calcola_momento_migliore_process, ev, data) {
    ...
    if (produzione_solare_predetta > consumo_predetto + consumo_lavatrice && produzione > consumo) {
...
PROCESS_THREAD(registra_dispositivo_process, ev, data) {
    ...
    leds_single_on(LEDS_YELLOW);//test!!
    ...
    while (!is_registered) {
        ...
        snprintf(msg, sizeof(msg), "{\"t\": \"actuator\", \"n\": \"Lavatrice\", \"c\": \"1.3\", \"d\": \"60\"}");
